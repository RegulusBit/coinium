...
###   This script attempts to download the signature file SHA256SUMS.asc from coinium.org
...
WORKINGDIR="/tmp/coinium"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://coinium.org/bin/"
VERSIONPREFIX="coinium-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'coinium-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Coinium Core binary release signing key installed?"
   fi
