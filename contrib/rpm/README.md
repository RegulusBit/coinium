...
The RPM spec file provided here is for Coinium-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://coinium.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Coinium git repository](https://github.com/coinium/coinium) but are not part of
...
Currently only `Source10` is used, for the example `coinium.conf` file.
...
in the coinium git repository. At present only an SVG version of the Coinium
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the coinium
...
needed when 0.12.1 is released, a proper fix is already in the Coinium git
master branch.
...
idea to build Coinium Core against a static version of that library compiled
...
The `coinium.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `coinium-server` package. At this point, most RPM based
...
The `coinium-server` package should have SELinux support. How to properly do
...
Coinium-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/coinium/). He has been
packaging Coinium for Fedora at least since 2012.
...
4. I renamed the `coinium` package that contains the Qt GUI to `coinium-core` as
that appears to be how the general population refers to it, in contrast to
`coinium-xt` or `coinium-classic`. I wanted to make sure the general population
